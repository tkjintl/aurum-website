...
async function opSpot(req, res) {
  if (req.method !== 'GET') return methodNotAllowed(res, ['GET']);
  const spot = await getKrwPerKg();
  // Add usd_per_kg for client gold-value calculations
  if (spot && spot.usd_per_oz) spot.usd_per_kg = Math.round(spot.usd_per_oz * 32.1507466);
  return ok(res, spot);
}
...
async function opPortfolio(req, res) {
  ...
  let spot = null;
  try { 
    spot = await getKrwPerKg();
    // Add usd_per_kg for client-side gold value calculations
    if (spot && spot.usd_per_oz) {
      spot.usd_per_kg = Math.round(spot.usd_per_oz * 32.1507466);
    }
  } catch {}
