...
function LaunchCalc({ activeTier, setActiveTier, prices = { gold: 3342.80 }, krwRate = 1368 }) {
  ...
  const SPOT_KRW_G = prices.gold * krwRate / 31.1035;
  ...
  return (
    <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}`, padding:'28px 28px', height:'100%', position:'relative', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      {/* FIX: fontSize:9→12 */}
      <div style={{ fontFamily:T.mono, fontSize:12, color:T.goldDim, letterSpacing:'0.26em', textTransform:'uppercase', marginBottom:22 }}>AGP LAUNCH ESTIMATOR</div>
      <div style={{ marginBottom:28 }}>
        {/* FIX: fontSize:9→12 */}
        <div style={{ fontFamily:T.mono, fontSize:12, color:T.goldDim, letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:14 }}>월 적립액</div>
        ...
        {/* FIX: fontSize:10→12 */}
        <div style={{ display:'flex', justifyContent:'space-between', marginTop:6, fontFamily:T.mono, fontSize:12, color:T.textMuted, letterSpacing:'0.12em' }}>
        ...
        {[
          ...
          <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', padding:'12px 0', borderBottom:`1px solid rgba(197,165,114,0.06)` }}>
            <div>
              ...
              {/* FIX: fontSize:11→13 */}
              <span style={{ fontFamily:T.serif, fontStyle:'italic', fontSize:13, color:T.textMuted, marginLeft:6 }}>{row.en}</span>
            </div>
      ...
      {/* FIX: fontSize:8→11 */}
      <div style={{ marginTop:20, paddingTop:16, borderTop:`1px solid ${T.border}`, display:'flex', justifyContent:'space-between', fontFamily:T.mono, fontSize:11, letterSpacing:'0.18em', color:T.textMuted, textTransform:'uppercase' }}>
...
function GMVBonusTrack({ navigate }) {
  return (
    <div style={{ background:T.bgCard, border:`1px solid ${T.goldBorder}`, padding:'28px 28px', height:'100%', position:'relative', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      <div style={{ position:'absolute', top:0, left:0, right:0, height:1, background:`linear-gradient(90deg,transparent,${T.gold},transparent)` }} />
      {/* FIX: fontSize:9→12 */}
      <div style={{ fontFamily:T.mono, fontSize:12, color:T.goldDim, letterSpacing:'0.26em', textTransform:'uppercase', marginBottom:8 }}>GMV GROWTH REWARDS · 성장 보너스</div>
      {/* FIX: fontSize:12→13 */}
      <p style={{ fontFamily:T.sansKr, fontSize:13, color:T.textSub, lineHeight:1.65, marginBottom:22 }}>
          ...
          <div key={i} style={{ display:'grid', gridTemplateColumns:'40px 1fr auto', alignItems:'center', gap:12, padding:'12px 0', borderBottom:i<GMV_BONUSES.length-1?`1px solid rgba(197,165,114,0.06)`:'none', position:'relative', zIndex:1 }}>
            ...
            <div>
              <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', marginBottom:3 }}>
                {/* FIX: fontSize:8→11 */}
                <span style={{ fontFamily:T.mono, fontSize:11, color:T.gold, border:`1px solid ${T.goldBorder}`, padding:'2px 6px', letterSpacing:'0.14em' }}>GATE {row.gate}</span>
                {/* FIX: fontSize:11→13 */}
                <span style={{ fontFamily:T.mono, fontSize:13, color:T.text, fontWeight:600 }}>{row.gmvKR}</span>
                {/* FIX: fontSize:10→12 */}
                <span style={{ fontFamily:T.sansKr, fontSize:12, color:T.textMuted }}>≈ {row.gmv}</span>
              </div>
              {/* FIX: fontSize:11→13 */}
              <div style={{ fontFamily:T.sansKr, fontSize:13, color:T.textSub, lineHeight:1.4 }}>{row.desc}</div>
            </div>
      ...
      {AGP_TIERS.map((tier,i)=>{
        ...
        return (
          <div key={i} onClick={()=>setActiveTier(i)} style={{ gridColumn:isMobile&&tier.featured?'1 / -1':'auto', background:tier.featured?`linear-gradient(180deg,${T.goldGlow},${T.bg})`:T.bg, border:`1px solid ${isActive?T.goldBorderStrong:tier.featured?T.goldBorder:T.border}`, padding:isMobile?(tier.featured?'24px 20px':'20px 12px'):'28px 16px', textAlign:'center', position:'relative', overflow:'hidden', cursor:'pointer', transform:isActive?'translateY(-4px)':'none', boxShadow:isActive?`0 10px 32px rgba(197,165,114,0.15)`:'none', transition:'all 0.3s cubic-bezier(0.2,0.8,0.2,1)' }}>
            ...
            {tier.featured && <div style={{ position:'absolute', top:10, right:10, fontFamily:T.mono, fontSize:10, color:T.bg, background:T.gold, padding:'2px 7px', letterSpacing:'0.18em' }}>추천</div>}
            ...
            {/* FIX: fontSize:10→12 */}
            <div style={{ fontFamily:T.serif, fontStyle:'italic', fontSize:12, color:T.goldDim, marginBottom:18 }}>{tier.nameEN}</div>
            <div style={{ height:1, background:T.goldBorder, margin:'0 0 18px' }} />
            {/* FIX: fontSize:8→11 */}
            <div style={{ fontFamily:T.mono, fontSize:11, letterSpacing:'0.2em', textTransform:'uppercase', color:T.textMuted, marginBottom:5 }}>LAUNCH GIFT</div>
            ...
            {/* FIX: fontSize:11→13 */}
            <div style={{ fontFamily:T.mono, fontSize:13, color:isActive?T.goldBright:T.textSub, fontWeight:600, marginBottom:16 }}>{tier.monthly}</div>
            <div style={{ paddingTop:14, borderTop:`1px solid ${T.border}` }}>
              {tier.perks.map((perk,pi)=>(
                <div key={pi} style={{ display:'flex', gap:6, alignItems:'flex-start', padding:'4px 0', textAlign:'left' }}>
                  ...
                  {/* FIX: fontSize:10→12 */}
                  <span style={{ fontFamily:T.sansKr, fontSize:12, color:T.textSub, lineHeight:1.4 }}>{perk}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ position:'relative' }}>
        ...
        {steps.map((row,i)=>(
          <div key={i} style={{ display:'grid', gridTemplateColumns:'48px 1fr', alignItems:'start', padding:'16px 0' }}>
            ...
            <div>
              <div style={{ fontFamily:T.mono, fontSize:11, letterSpacing:'0.22em', textTransform:'uppercase', color:T.goldDim, marginBottom:4 }}>{row.when}</div>
    ...
    <div style={{ background:`radial-gradient(ellipse at 50% 0%,rgba(197,165,114,0.12),transparent 65%),${T.bg2}`, border:`1px solid ${T.goldBorderStrong}`, padding:'36px 32px', display:'flex', flexDirection:'column', justifyContent:'center', position:'relative', overflow:'hidden' }}>
      ...
      {submitted ? (
        ...
        <form onSubmit={handleReserve}>
          ...
          <div style={{ display:'flex', gap:18, flexWrap:'wrap', fontFamily:T.mono, fontSize:11, letterSpacing:'0.18em', color:T.textMuted, textTransform:'uppercase' }}>
...
export default function AGPLaunchPage({ lang, navigate, user, setShowLogin, prices = { gold: 3342.80 }, krwRate = 1368 }) {
  ...
  return (
    <div style={{ background:T.bg }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'repeat(4,1fr)', gap:0 }}>
          {[{val:'5단계',label:'론치 기프트 티어'},{val:'₩50K~₩5M',label:'티어별 실물 금 기프트'},{val:'30일',label:'잠금 해제 후 영구 소유'},{val:'GMV+',label:'성장할수록 추가 크레딧'}].map((s,i)=>(
            <div key={i} style={{ textAlign:'center', padding:isMobile?'14px 8px':'20px 16px', borderRight:!isMobile&&i<3?`1px solid ${T.goldBorder}`:'none', borderBottom:isMobile&&i<2?`1px solid ${T.goldBorder}`:'none' }}>
              ...
              <div style={{ fontFamily:T.sans, fontSize:isMobile?9:13, color:T.textMuted, marginTop:5 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ maxWidth:1000, margin:'0 auto', display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 60px 1fr 60px 1fr', gap:0, alignItems:'start' }}>
          {[
            ...
            <div key={i} style={{ textAlign:'center', padding:'0 10px' }}>
              ...
              {/* FIX: fontSize:15→18 */}
              <div style={{ fontFamily:T.serifKr, fontSize:18, fontWeight:500, color:T.text, marginBottom:6 }}>{s.kr}</div>
              {/* FIX: fontSize:11→13, maxWidth:180→200 */}
              <div style={{ fontFamily:T.sansKr, fontSize:13, color:T.textSub, lineHeight:1.65, maxWidth:200, margin:'0 auto' }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:pad, borderBottom:`1px solid ${T.border}` }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:44 }}>
            {/* FIX B2-27: fontSize:9→13, letterSpacing:0.3em→0.18em */}
            <div style={{ fontFamily:T.mono, fontSize:13, color:T.gold, letterSpacing:'0.18em', textTransform:'uppercase', marginBottom:12 }}>5 Tiers · 5가지 티어</div>
      ...
      {/* ── Calculator + GMV Bonus ── */}
      <div style={{ padding:pad, background:T.bg1, borderBottom:`1px solid ${T.border}` }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:40 }}>
            {/* FIX B2-67: fontSize:9→13 */}
            <div style={{ fontFamily:T.mono, fontSize:13, color:T.gold, letterSpacing:'0.18em', textTransform:'uppercase', marginBottom:12 }}>Launch Estimator + GMV Growth</div>
